...
def main(search_regex, replace_regex, file_path, in_place, lines):
    if not os.path.exists(file_path):
        click.error(f"file does not exist: {file_path}")
    if not os.path.isfile(file_path):
        click.error(f"path is not a file: {file_path}")
    if not os.access(file_path, os.R_OK):
        click.error(f"unable to file for reading: {file_path}")
    ...
    if in_place:
        if not os.access(file_path, os.W_OK):
            click.error(f"unable to file for writing: {file_path}")
