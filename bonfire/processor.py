...
class TemplateProcessor:
    ...
    def _process_component(self, component_name, app_name, in_recursion, dependency_chain):
        if component_name in self.processed_components:
            ...
            # Assume the following graph where o() indicates an optional dependency.
            # A -> o(B) -> o(C)
            # B -> o(C)
            # And assume "hybrid" is used as the optional dependency resolution method.  They
            # "hybrid" method is basically "don't add optional dependencies of optional
            # dependencies".
            #
            # In this case, if we deploy both apps A and B, the hybrid resolution model means that
            # by the time we process app B, it has already been processed as an optional
            # dependency of A and C has been *skipped*.  We need to reprocess the dependencies to
            # ensure any optional dependencies that were skipped are now handled.
