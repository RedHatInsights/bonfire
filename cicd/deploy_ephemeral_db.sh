...
# shellcheck source=cicd/_common_deploy_logic.sh
...
export DB_NAMESPACE="$NAMESPACE"  # track which namespace was used here for 'teardown' in common_deploy_logic
...
DATABASE_NAME=$(jq -r .name < db-creds.json)
export DATABASE_NAME
DATABASE_ADMIN_USERNAME=$(jq -r .adminUsername < db-creds.json)
export DATABASE_ADMIN_USERNAME
DATABASE_ADMIN_PASSWORD=$(jq -r .adminPassword < db-creds.json)
export DATABASE_ADMIN_PASSWORD
DATABASE_USER=$(jq -r .username < db-creds.json)
export DATABASE_USER
DATABASE_PASSWORD=$(jq -r .password < db-creds.json)
